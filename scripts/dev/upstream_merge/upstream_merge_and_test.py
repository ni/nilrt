...
def build_and_test(clean_build, vm_name, snapshot_name):
    """
    ...
    :param vm_name: Name of the virtual machine to use for testing.
    :param snapshot_name: Name of the VM snapshot to restore before testing.
    :return: A tuple (status_code, message).
            Returns (0, None) on success, or error details on failure.
    """
    success = setup_env_and_build_packages("--org", clean_build)
    if success[0] != 0:
        return success
    success = install_and_test_image(vm_name, snapshot_name)
    return success
...
def main():
    ...
    if merge_has_errors:
        ...
        build_and_test_details = build_and_test(
            ...
            vm_name=json_config_obj.vm_name,
            snapshot_name=json_config_obj.snapshot_name,
        )
