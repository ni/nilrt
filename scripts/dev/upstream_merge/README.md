...
- Installs and tests these images on a virtual machine to verify that the integration is successful and the target works as expected.
...
### **2. Run the Script**
#### **To Perform a Merge**
This will merge upstream changes, build the images, and test them and creates PR with the new changes after the build and test steps succeed.
...
```bash
python3 scripts/dev/upstream_merge/upstream_merge_and_test.py -w workItemID
```
...
#### **To Skip the Merge and Only Build and Test and create PRs**
...
If a merge conflict occurs, you will need to log into the build machine where the merge automation runs, manually resolve the conflict in the relevant repository, and commit the changes. Access to this machine is restricted. To obtain credentials or instructions for accessing the build machine, please contact Shreejit C or Pratheeksha S N (see Contact section below).
...
```bash
python3 scripts/dev/upstream_merge/upstream_merge_and_test.py -w workItemID -s
```
...
```bash
python3 scripts/dev/upstream_merge/upstream_merge_and_test.py -w workItemID -skip_push_and_pr
```
...
> **Important Note:**<br>
> Currently, the VM and the snapshot required for this script exist only on one of our local machines. As a result, the script can only be run on this machine. In the future, we plan to set up a YAML-based script (or a similar automation tool) to run the entire process on this machine.
...
- Key steps include:
  - Restoring the VM to a clean point with SSH enabled.
